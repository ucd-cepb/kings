...
create_dac_meta <- function(type, scope){
   ...
   #each list item is a different gsp
      ...
      #proportion of place in each census tract = area of census_dac intersection / dac area
      area_overlap = st_area(st_intersection(dacplace[i,],censusplot[place_census_overs[[i]],]))
      prop_place_in_tract = area_overlap/st_area(dacplace[i,])
      prop_tract_in_place = area_overlap/st_area(censusplot[place_census_overs[[i]],])
      data.table(census_tract_id = censusplot$FIPS[place_census_overs[[i]]],
                 dac_place_id = dacplace$GEOID[i],
                 population = censusplot$E_TOTPOP[place_census_overs[[i]]],
                 dacpop = dacplace$Pop18[i],
                 SVI_raw = ifelse(censusplot$SPL_THEMES[place_census_overs[[i]]]>=0,
                                  censusplot$SPL_THEMES[place_census_overs[[i]]],NA),
                 SVI_percentile = ifelse(censusplot$RPL_THEMES[place_census_overs[[i]]]>=0,
                                         censusplot$RPL_THEMES[place_census_overs[[i]]],NA),
                 DAC = ifelse(dacplace$DAC18[i] == "Y", T, 
                              ifelse(dacplace$DAC18[i]=="N",F,NA)),
                 Prop_place_in_tract = as.numeric(prop_place_in_tract),
                 Prop_tract_in_place = as.numeric(prop_tract_in_place)
                 #return entries where over 0.5 percent of the census tract is in the dac tract
                 #as well as entries where over 0.5 percent of the dac tract is made up of the census tract
      )[Prop_tract_in_place >= 0.005 | Prop_place_in_tract >= 0.005]
   }, cl = 4)
