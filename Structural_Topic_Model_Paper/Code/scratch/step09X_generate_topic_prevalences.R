...
model0 <- readRDS("data/Structural_Topic_Model_Paper/mdl/model_20240306-09:31")
model1 <- readRDS("data/Structural_Topic_Model_Paper/mdl/model_20240318-14:59" )
model0$settings$covariates$formula
model1$settings$covariates$formula

# findTopic doens;'t handle regex
# do it ourselves
frex_scores0 <- labelTopics(model0,n = 50)$frex
frex_scores1 <- labelTopics(model1,n = 50)$frex
...
topic_ids0 <- lapply(topic_indicators,function(indicator){
...
topic_ids1 <- lapply(topic_indicators,function(indicator){
   word_match_matrix <- matrix(grepl(paste(indicator,collapse = '|'),frex_scores),
                               byrow = F,ncol = ncol(frex_scores))
   topic_nums <- which(rowSums(word_match_matrix)>0)
   topic_nums})
...
cc_interaction0 = stm::estimateEffect(formula = c(topic_ids0$cc) ~ Republican_Vote_Share_scaled * maxdryspell_scaled,
                                    metadata = model0$settings$covariates$X,
                                    stmobj = model0)
ej_interaction1 = estimateEffectDEV(formula = topic_ids1$cc ~ Republican_Vote_Share_scaled * maxdryspell_scaled,
                                    metadata = model1$settings$covariates$X,group = T,
                                    stmobj = model1)
head(model1$settings$covariates$X)
par(mfrow=c(1,2))
plot(cc_interaction0, "maxdryspell_scaled", method="continuous",
     xlim=c(-2,2), moderator="Republican_Vote_Share_scaled", moderator.value=c(-1))
plot(cc_interaction0, "maxdryspell_scaled", method="continuous",
     xlim=c(-2,2), moderator="Republican_Vote_Share_scaled", moderator.value=c(1))
