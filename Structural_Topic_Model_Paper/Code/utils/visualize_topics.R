...
#note this function is not set up to handle group topics
   ...
   if(effects ==T){
      ...
      saveRDS(effect,filekey[filekey$var_name=="estimate_effects_stmpaper",]$filepath)
      estimate_effs <- readRDS(filekey[filekey$var_name=="estimate_effects_stmpaper",]$filepath)
      set.seed(43)
      ...
      for(i in 1:length(sumef$tables)){
         ...
         efbytopic <- as.data.table(cbind("Factors" = rownames(sumef$tables[[i]]),
                                          sumef$tables[[i]]))
         write_csv(efbytopic, file = paste0(filekey[filekey$var_name=="effect_table_csvs_stmpaper",]$filepath,sumef$topics[i],"_",categ_no_m_na[i],".csv"))
      }
