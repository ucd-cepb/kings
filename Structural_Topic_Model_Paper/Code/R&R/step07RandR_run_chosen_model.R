...
gsp_out$meta <- left_join(gsp_out$meta, boardsummary)       
...
while (!gsp_model$convergence$converged){
   gsp_model <- stm(documents = gsp_out$documents, vocab = gsp_out$vocab,
                    ...
                    data = gsp_out$meta,
                    model = gsp_model)
   saveRDS(gsp_model, paste0("RandRpartialmodel_",format(Sys.time(), "%Y%m%d-%H:%M")))
}




saveRDS(gsp_model, file = paste0("RandRmodel_",format(Sys.time(), "%Y%m%d-%H:%M")))
