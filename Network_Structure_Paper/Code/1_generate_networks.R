...
# Load environment variables and read the edgelist with metadata
...
ve <- readRDS(edge_fp) %>% 
   ...
   filter(unlist(lapply(seq_along(type_id), function(k) !is.null(type_id[[k]])))) %>% # keep verbnet verbs
   select(-c(2:10)) %>% 
...
aggregate_gsas <- function(networklist, gsp_id) {
...
net_process <- function(file, gsp_id){
   ...
   el <- ve %>% filter(GSP_ID == as.numeric(gsp_id))
...
net_graph <- function(networklist, gsp_id) {
   ...
   networklist <- aggregate_gsas(networklist, gsp_id)
   ...
   # attributes
   network::set.vertex.attribute(network_graph, 'GSA', 
                                 ifelse(network.vertex.names(network_graph) %in% gsa_names2, 1, 0))
   gsa_indices <- which(network::get.vertex.attribute(network_graph, 'GSA') == 1)
   network::set.vertex.attribute(network_graph, 'org_type', value = 'GSA', v = gsa_indices)
