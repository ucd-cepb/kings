...
```{r}
mod_names <- c('exists_mod', 'exists_mod_2', 'eig_mod', 'eig_mod_2', 'lead_mod', 'lead_mod_2')
mods <- list(exists_mod, exists_mod_2, eig_mod, eig_mod_2, lead_mod, lead_mod_2)

coef_data <- data.frame()

for (i in seq_along(mods)) {
   coef <- tidy(mods[[i]], conf.int = TRUE, conf.level = 0.90) %>%
      filter(term %in% c('POP_std', 'incorporated', 'per_latino', 'Intercept'))
   coef$mod_name <- mod_names[i]
   coef_data <- rbind(coef_data, coef)
}
```
...
b_dac <- lm(basin_plan_per ~ DAC+
               POP_std+
               incorporated+
               per_latino,
              data = all_place_nodes)

c_dac <- lm(sust_criteria_per ~ DAC+
               POP_std+
               incorporated+
               per_latino,
              data = all_place_nodes)

d_dac <- lm(monitoring_networks_per ~ DAC+
               POP_std+
               incorporated+
               per_latino,
              data = all_place_nodes)

e_dac <- lm(projects_mgmt_actions_per ~ DAC+
               POP_std+
               incorporated+
               per_latino,
              data = all_place_nodes)
