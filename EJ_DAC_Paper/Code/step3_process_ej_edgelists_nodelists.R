...
gsp_tblfilename <- filekey[filekey$var_name=="gsp_web_vars_repaired_ejpaper",]$filepath
...
pdftxt <- readRDS(filekey[filekey$var_name=="cleaned_pdfs_for_ejpaper",]$filepath)
...
for(m in 1:length(edges_and_nodes)){
   ...
   saveRDS(edgenodelist,paste0(filekey[filekey$var_name=="disambiged_extracts_ejpaper",]$filepath,"/",gspids[m],".RDS"))
}

for(m in 1:length(edges_and_nodes)){
   edgenodelist <- readRDS(paste0(filekey[filekey$var_name=="disambiged_extracts_ejpaper",]$filepath,"/",gspids[m],".RDS"))
...
###Section 5: Network Object Generation####
   ...
   saveRDS(full_directed_graph, paste0(filekey[filekey$var_name=="full_directed_graphs_ejpaper",]$filepath,gspids[m]))
