...
for(m in list){
  centr_df <- readRDS(paste0("data_output/node_attr_",gspids[m]))
...
for(m in list){
  ...
  adj_mat <- readRDS(paste0("data_output/adjmat_orig_",gspids[m]))
  ...
  saveRDS(binary_sq, paste0("data_output/binarymat_sq_",gspids[m]))
  
  triangles <- binary_mat * binary_sq
  saveRDS(triangles, paste0("data_output/triangles_",gspids[m]))
  ...
  saveRDS(centr_df, paste0("data_output/node_attr_tr_",gspids[m]))
...
for(m in list){
  ...
  centr_df <- readRDS(paste0("data_output/node_attr_tr_",gspids[m]))
  ##SUBSETTING FOR POL 279 ASSIGNMENT 2 
  centr_df <- centr_df %>% filter(type %in% c("ORG","GPE","PERSON"))
