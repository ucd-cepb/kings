gsp_pdf_reader<- function(box_sync = F){
   ...
   for(k in 1:length(pdfs)){
      ...
      if(!file.exists(paste0("data_cleaned/",substr(pdfs[k],10,24),"_text"))){
         ...
         saveRDS(text, file = paste0("data_cleaned/",substr(pdfs[k],10,24),"_text"))

         page_key <- create_page_key(paste0(substr(pdfs[[k]],1,24),".xlsx"))
         saveRDS(page_key, file = paste0("data_cleaned/",substr(pdfs[k],10,24),"_categories"))
