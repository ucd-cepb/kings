...
to_lower = T
   ...
   agency_names <- as_tibble(read_csv("textnet_demo_data/agency_list.csv")) %>% 
   ...
   if(to_lower==T){
      place_names <- as_tibble(read.table(
         "textnet_demo_data/2021_Gaz_place_national.txt",
   ...
   cnty_names <- as_tibble(read.table("textnet_demo_data/2021_Gaz_counties_national.txt",
                                      sep="\t", quote = "", header = TRUE)) %>% filter(USPS == "CA")
...
saveRDS(names,file = 'textnet_demo_data/cleaned_agency_list.rds')
