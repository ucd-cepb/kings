#### this symbolic link points to box drive ####
### long term, we want to create functionality where:
#1. user specifies file locations (or says 'just keep in memory, no save')
#2. that file location is in a project folder, not package folder

#system('ln -s ~/Library/CloudStorage/Box-Box/Kings_Large_Files/* data/')
system('ln -s ~/Library/CloudStorage/Box-Box/Kings_Large_Files/textnet_data/* data/')
...
#### NOTE: FOR FUTURE, DON'T INCLUDE THIS -- FOLKS WILL USE DIFFERENT PYTHON BUILDS AND MANAGERS
...
for (m in 1:length(gspids)){
  ...
  if(draw_edges==TRUE){
      if(!parsefileexists){
        ...
        if(mini_data==T){
          single_plan_text <- single_plan_text[25:31]
        }
      ...
      #TODO this workflow is now different. Splits off for directional
      if(directed){
        custom_entity_extractV2(parsedtxt,concatenator="_",gspids[m],cl = 8)
      }
      if(!directed){
        ...
        entities_valid <- entities[doc_sent %in% valid_sentences$doc_sent,]
        rm(entities)
        ...
        new_num_mentions <- entities_unique[,sum(num_mentions),by=.(entity)] 
