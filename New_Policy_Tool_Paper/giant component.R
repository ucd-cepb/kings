...
for (i in 1:length(file_paths)) {
  ...
  # weighted g
  weighted_graph <- graph_from_data_frame(d = edges_with_weights, vertices = nodelist_filtered, directed = TRUE)
  
  #extract the giant component
  components <- igraph::components(weighted_graph, mode = "weak")
  giant_component_id <- which.max(components$csize)
  vertex_ids <- V(weighted_graph)[components$membership == giant_component_id]
  giant_component <- igraph::induced_subgraph(weighted_graph, vertex_ids)
  
  
  V(giant_component)$color <- ifelse(V(giant_component)$entity_type == "GPE", "#C3B1E1",  
                       ifelse(V(giant_component)$entity_type == "ORG", "#A2D5C6",  
                              ifelse(V(giant_component)$entity_type == "PERSON", "#FFCBA4",  
                                     "#D3D3D3")))  
  V(giant_component)$size <- 3  
  ...
  plot(giant_component, 
       ...
       vertex.size = V(giant_component)$size,    
...
############## Giant component network statistics #################
...
for (i in 1:length(file_paths)) {
  ...
  giant_component_id <- which.max(components$csize)
  vertex_ids <- V(weighted_graph)[components$membership == giant_component_id]
  g <- giant_component <- igraph::induced_subgraph(weighted_graph, vertex_ids)
...
ggplot(cor_melt, aes(x = Var1, y = Var2, fill = value)) +
  ...
  labs(title = "Giant Component Heatmap(weighted)", x = "", y = "")
